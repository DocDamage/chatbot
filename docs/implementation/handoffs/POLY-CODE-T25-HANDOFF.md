...
- Implementation commits: `cff12d6`, `9af1d6e`, `383e195`, `d5ec608`, and `54623a7`.
...
- The checked-in live evidence bundle records an explicit `gemini-3.6-flash` run at `54623a7`: 13 ready and 14 unsupported toolchains, five model-adapter cases, and honest provider-quota failures for later cases. A separate `deepseek-chat` run reached the ready cases on the preceding implementation; the earlier `gemini-2.0-flash` attempt was rejected because that model was retired.
