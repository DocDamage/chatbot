...
export async function openAdvancedWorkspace(page) {
  const settings = page.getByRole('button', { name: 'Open settings' });
  await settings.click();
  const dialog = page.getByRole('dialog', { name: 'Settings' });
  await expect(dialog).toBeVisible();
  const toggle = dialog.getByRole('button', { name: /Open advanced workspace|Hide advanced workspace/ });
  if ((await toggle.innerText()).includes('Open')) {
    await toggle.click();
  } else {
    await dialog.getByRole('button', { name: 'Close settings' }).click();
  }
  await expect(page.getByRole('region', { name: 'Conversation tools' })).toBeVisible();
}
