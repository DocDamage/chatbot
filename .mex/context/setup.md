...
- Optional PostgreSQL/Redis, depending on configured persistence and cache modes.
...
- `npm run type-check` — server, tests, and client type-checks.
- `npm test` — Jest suite.
- `npm run lint` — server and client linting.
- `npm run build` — production server and client build.
- `npm run release:check` — release validation and smoke checks.
- `npm run export:skill -- docs .agents/skills/project-knowledge` — package docs as an on-demand skill.
- `npx mex-agent graph` — rebuild the local code graph.

Package scripts also include `type-check:server`, `type-check:tests`, `type-check:client`, `smoke:package`, `eval:rag`, `eval:safety`, `eval:all`, `eval:math`, `eval:market`, `eval:gamedev`, `eval:genius`, `eval:sixsigma`, `eval:blackbelt`, `eval:popculture`, `eval:history`, `eval:science`, `eval:chrono`, `eval:music`, `eval:suno`, `eval:daw`, `eval:story`, `eval:legal`, `eval:health`, `eval:security`, `eval:business`, `eval:creative`, `eval:roleplay`, `eval:philosophy`, `eval:language`, `eval:geography`, `eval:engineering`, `eval:nlu`, `eval:knowledge-os`, `kb:backup`, and `kb:ocr-queue`.
